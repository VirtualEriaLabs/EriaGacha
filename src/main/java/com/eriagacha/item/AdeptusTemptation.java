...
import static com.eriagacha.utils.RegisterUtils.status;
  ...
  public AdeptusTemptation() {
    super(new Item.Settings().group(ItemGroup.MISC)
        .food(new FoodComponent.Builder().hunger(20).saturationModifier(40f).snack().alwaysEdible()
            .statusEffect(status(StatusEffects.SPEED, 20000, 6), 2)
            .statusEffect(status(StatusEffects.HASTE, 20000, 8), 2)
            .statusEffect(status(StatusEffects.NIGHT_VISION, 20000, 0), 2)
            .statusEffect(status(StatusEffects.REGENERATION, 20000, 2), 2)
            ...
            .statusEffect(status(StatusEffects.STRENGTH, 20000, 8), 2)
            .statusEffect(status(StatusEffects.FIRE_RESISTANCE, 20000, 1), 2)
